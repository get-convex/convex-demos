...
const WebSocket: typeof window.WebSocket =
...
class Connection {
  ...
  private tokenMap: Record<string, Subscription>;
   ...
   */
  private dispose(subscription: Subscription) {
   ...
   */
  private async poll(subscription: Subscription) {
    ...
    try {
      ...
      console.log(
        `Error in data binding fetch call: ${error};\nBacking off for ${backoff}ms`
      );
      setTimeout(() => {
        this.poll(subscription);
      }, backoff);
    ...
    subscription.token = result.token as string;
   ...
   */
  private sendToken(token: string, begin: boolean) {
    if (this.connected) {
 ...
 */
class Subscription {
  /** Callable that produces a value and cursor information from a read-only query. */
  query: () => Promise<any>;
  /** Callback provided with new values of the query. */
  update: (value: any) => void;
  /** Current token for invalidation that is being waiting on the WebSocket, if any. */
  token: string | null;
   ...
   */
  constructor(query: () => any, update: (value: any) => void) {
    this.query = query;
    this.update = update;
    this.token = null;
    this.disposed = false;
